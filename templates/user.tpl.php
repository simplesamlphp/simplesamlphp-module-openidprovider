...
if ($userId !== false) {
    $title = $this->t('{openidProvider:openidProvider:title_user}', ['%USERID%' => htmlspecialchars($userId)]);
} else {
...
if ($userId !== false) {
    echo '<p>'.$this->t('{openidProvider:openidProvider:user_page_for}', ['%USERID%' => htmlspecialchars($userId)]).'</p>';
}
...
if ($ownPage) {
    ...
    if (count($trustedSites) > 0) {
        ...
        foreach ($trustedSites as $site) {
            echo '<li>';
            echo '<input type="submit" name="remove_'.bin2hex($site).'" value="'.
                $this->t('{openidProvider:openidProvider:trustlist_remove}').'" />';
